...
class LoginViewApi(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if user:
            ...
            return Response(
                {"message": "Неверный логин либо парроль"},
...
class RegistrationViewApi(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if not username or not phone or not password1 or not password2:
            return Response(
                {"error": "Не заповнені усі поля"}, status=status.HTTP_400_BAD_REQUEST
            )

        if password1 != password2:
            return Response(
                {"error": "Пароли не співпадають"}, status=status.HTTP_400_BAD_REQUEST
            )

        if len(phone) < 10 or len(phone) > 15:
            return Response(
                {"error": "Невірний номер телефона"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if re.match(pattern, phone):
            ...
            if User.objects.filter(Q(username=username) | Q(phone=phone)).exists():
                return Response(
                    {
                        "error": "Користувач з таким логіном або номером телефону вже існує"
                    },
...
class ProductMenuPageAPI(APIView):
    def get(self, request, *args, **kwargs):
        ...
        if category:
            products = products.select_related("category").filter(
...
class ProductCreateViewAPI(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response(
                {"message": "Продукт успешно создан", "product": serializer.data},
...
class CoffeehousesMapAPIView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        serializer = serializers.CoffeehousesMapSerializer(
            queryset, many=True, context={"request": request}
        )
...
class ReservationSearchAPI(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not phone:
            return Response(
                {"error": "По цьому номеру немає резервацій"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if not re.match(pattern, phone):
            return Response(
                {
                    "error": "Невірний номер телефону, перевірте будь ласка правильність введеного номеру"
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if actual:
            ...
            reservations = (
                Reservation.objects.filter(customer_phone=phone)
                .select_related("coffeehouse", "table")
    ...
    def get(self, request, *args, **kwargs):
        ...
        reservations = (
            Reservation.objects.filter(
                Q(customer_name=user.username) | Q(customer_phone=user.phone)
            )
            .exclude(id__in=actual_res_list)
            .select_related("coffeehouse")
            .order_by("-reservation_date")[:3]
        )

        serializer_actual_res = serializers.ReservationProfileSericalizer(
            actual_res_queryset, many=True
        )
        serializer_reservations = serializers.ReservationProfileSericalizer(
        ...
        return Response(
            {
                "reservations": serializer_reservations.data,
                "actual_reservations": serializer_actual_res.data,
                "user_data": user_data,
            },
...
class ProfileHitoryAPI(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if result_page is not None:
            serializer = serializers.ReservationProfileSericalizer(
                result_page, many=True, context={"request": request}
            )
            return paginator.get_paginated_response(serializer.data)
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            closing_time = datetime.strptime(
                coffeehouse.closing_time.strftime("%H:%M"), "%H:%M"
            )
            ...
            print("Ошибка", e)
            return Response(
                {"error": f"Ошибка логики {e}"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            reservations_today = Reservation.objects.filter(
                coffeehouse_id=coffeehouse_id, reservation_date=reservation_date
            ...
            reservation_datetime = datetime.combine(
                datetime.strptime(reservation_date, "%Y-%m-%d"), reservation_time_obj
            )
            ...
            overlapping_reservations = (
                annotations_reservation.filter(end_time__gt=reservation_time)
                .filter(reservation_time__lt=end_time)
                .values_list("table_id", flat=True)
            )

            # Формируем данные для ответа
            tables_data = [
                {"id": table.id, "name": table.table_number}
                for table in Table.objects.filter(
                    coffeehouse_id=coffeehouse_id
                ).exclude(id__in=overlapping_reservations)
            ]
            ...
            print("ОШИБКА", e)
            return Response(
                {"error": f"Invalid JSON data {e}"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class CreateReservationAPI(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer = serializers.ReservationCreateSerializer(
            data=request.data, context={"request": request, "ip": ip}
        )
        ...
        if serializer.is_valid():
            ...
            return Response(
                {
                    "message": "Бронирование успешно созданно",
...
class CoffeehouseFormApiView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        if serializer:
            return Response(
                {"coffeehouses": serializer.data}, status=status.HTTP_200_OK
            )
...
class LogoutTokenAPIView(APIView):
    def post(self, request, *args, **kwargs):
        response = Response({'message': 'Выход успешный'}, status=status.HTTP_200_OK)
